...
let counter = 0;
let currentStreak = { streak: [], emote: [], timers: [], emoteURL: [] }; // the current emote streak being used in chat
...
let minStreak = 5;//getUrlParam("minStreak", 5) > 2 ? getUrlParam("minStreak", 5) : 5; // minimum emote streak to trigger overlay effects (Minimum value allowed is 3)
...
function findEmotes(message, messageFull) {
    if (emotes.length !== 0) {
      ...
      //if it passes more then the starttimer the emote is taken from the currentStreak
      if(currentStreak.timers[currentStreak.timers.length - 1] > starttimer * 1000){
        aux = new Date().getTime();
        viewers.push(messageFull[14].match(/\d+/g)[0]);
        currentStreak.streak[currentStreak.emote.indexOf(emoteUsed)] = 0;
        currentStreak.emote[currentStreak.emote.indexOf(emoteUsed)] = null;
        currentStreak.timers[currentStreak.emote.indexOf(emoteUsed)] = 0;
      }
      if (messageSplit.includes(currentStreak.emote.find(element)) /*&& !(viewers.includes((messageFull[14].match(/\d+/g)[0])))*/) {
          aux = new Date().getTime();
          if(aux < counter + (timer * 1000)){
            viewers.push(messageFull[14].match(/\d+/g)[0]);
            currentStreak.streak[currentStreak.emote.indexOf(emoteUsed)]++;
          ...
          if(aux > counter + (timer * 1000)){
            ...
            viewers = [];
            currentStreak.streak.push(1);
            currentStreak.emote.push(message.substring(parseInt(emoteUsed.split(":")[1].split("-")[0]), parseInt(emoteUsed.split(":")[1].split("-")[1]) + 1));
            currentStreak.emoteURL.push(`https://static-cdn.jtvnw.net/emoticons/v2/${emoteUsed.split(":")[0]}/default/dark/2.0`);
            currentStreak.timers.push(new Date().getTime());
          }
      } else{
          aux = new Date().getTime();
          if(aux > counter + (timer * 1000)){
            ...
            viewer = [];
            currentStreak.streak.push(1);
            currentStreak.emote.push(findEmoteInMessage(messageSplit));
            currentStreak.emoteURL.push(findEmoteURLInEmotes(currentStreak.emote));
            currentStreak.timers.push(new Date().getTime());
          }
      }
